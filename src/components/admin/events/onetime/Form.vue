<template>
  <form @submit.prevent="handleSubmit" class="space-y-4">
    ...
    <div>
      ...
      <VueDatePicker v-model="form.date" :inline="true" :time-picker-inline="true" :is-24="true" 
        :auto-apply="true" :no-hours-overlay="true" :no-minutes-overlay="true"></VueDatePicker>
    </div>

    <div>
      <label for="price" class="block text-sm font-medium text-gray-700">Price</label>
      <input type="number" required v-model="form.price" min="10.00" step="0.50"
        class="mt-1 block w-full border rounded shadow-sm p-2" />
    </div>
