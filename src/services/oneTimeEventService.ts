...
export async function createOneTimeEvent(oneTimeEvent: oneTimeEvent): Promise<OneTimeEvent> {
...
export async function updateOneTimeEvent(oneTimeEvent: OneTimeEvent): Promise<OneTimeEvent> {
  const response = await api.put(`/api/events/onetime/${oneTimeEvent.id}`, oneTimeEvent);
  return response.data;
}

export async function deleteOneTimeEvent(oneTimeEvent: OneTimeEvent): Promise<void> {
